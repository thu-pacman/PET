...
def get_workload_dense(M, N, K, B, dtype):
    shape = {
        "A": (M, K),
        "B": (N, K),
    }
...
def get_workload(M, N, K, dtype):
    shapes = {
        "A": (M, K),
        "B": (N, K),
    }
    A = relay.var("A", shape=shapes["A"], dtype=dtype)
    B = relay.var("B", shape=shapes["B"], dtype=dtype)
    out = relay.nn.dense(A, B)
    params = {}  # No parameters are folded
    return tvm.IRModule.from_expr(relay.Function([A, B], out)), params, shapes
...
input_tasks = [  # Input, Kernel, p, s, d
    ...
    [515, 771, 771, 1],
    [516, 772, 772, 1],
    # [[1, 256, 18, 18], [384, 256, 1, 1], [0, 0], [1, 1], [1, 1]],  # group conv
    # [[1, 256, 18, 18], [320, 256, 1, 1], [0, 0], [1, 1], [1, 1]],  # group conv
    # [[1, 256, 18, 18], [384, 256, 1, 1], [0, 0], [1, 1], [1, 1]],  # group conv
    # [[1, 256, 18, 18], [320, 256, 1, 1], [0, 0], [1, 1], [1, 1]],  # group conv
]
...
for task_id in ran:
    ...
    # op, params, data_shape = get_workload_dense(M, N, K, B, dtype)
    # op, params, data_shape = get_workload(M, N, K, dtype)
    ...
    # op, params, data_shape = get_workload_conv2d(
    #     N, I, O, H, W, K, strides, padding, dilation, groups, dtype)
    # op = tvm.IRModule.from_expr(op)

    log_file = "relay_dense"+str(atask)+'.log'
