...
int main(int argc, char *argv[]) {
    ...
    const int inputDims[5] = {/* N = */ batchSize, /* C = */ atoi(argv[3]),
    ...
    const int n = inputDims[0], c = inputDims[1], h = inputDims[2],
              w = inputDims[3], z = inputDims[4];
    const int inSize = n * c * h * w * z;
    ...
    input->setDimensions(input_dims);

    builder->setMaxBatchSize(n);
    // Create an optimization profile and set the dimension as below
    IBuilderConfig *config = builder->createBuilderConfig();
    IOptimizationProfile *profile = builder->createOptimizationProfile();
    profile->setDimensions("input", OptProfileSelector::kMIN, input_dims);
    profile->setDimensions("input", OptProfileSelector::kOPT, input_dims);
    profile->setDimensions("input", OptProfileSelector::kMAX, input_dims);
