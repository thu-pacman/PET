...
int getOffset(std::vector<int> index, std::vector<int> shape) {
    ssize_t ret = index[0];
...
void tconv_compute_test1() {
    ...
    Dim dm = {0, 1, 2, 3};
    DimRange dr = DimRange(dm);
    std::vector<DimRange> inDr;
    std::function<bool()> runner;
    std::tie(inDr, runner) = convTransOp.compute(dr);
    ...
    assert(runner());
    assert(convTransOp.getOutput()->getData(dm) == ans[getOffset(dm, {1, 2, 4, 4})]);
}
...
void tconv_compute_test2() {
    ...
    assert(runner());
    assert(convTransOp.getOutput()->getData(dm) == ans[getOffset(dm, {1, 2, 2, 2})]);
}
