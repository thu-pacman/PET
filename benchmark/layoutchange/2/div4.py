...
output_tensor = (batch*4, in_channel, in_size//2 + pad*2, in_size//2 + pad*2)
...
A_ch = te.compute(
    output_tensor,
        lambda n, c, h, w: tvm.tir.if_then_else(
            n%4 == 0, tvm.tir.if_then_else(tvm.tir.all(h>=pad, w>=pad), A[n//4, c, h-pad, w-pad], tvm.tir.const(0., "float32")), tvm.tir.if_then_else(
            n%4 == 1, tvm.tir.if_then_else(tvm.tir.all(h>=pad, w<in_size//2+pad), A[n//4, c, h-pad, w+in_size//2-pad], tvm.tir.const(0., "float32")), tvm.tir.if_then_else(
            n%4 == 2, tvm.tir.if_then_else(tvm.tir.all(h<in_size//2+pad, w>=pad), A[n//4, c, h+in_size//2-pad, w-pad], tvm.tir.const(0., "float32")), tvm.tir.if_then_else(tvm.tir.all(h<in_size//2+pad, w<in_size//2+pad), A[n//4, c, h+in_size//2-pad, w+in_size//2-pad], tvm.tir.const(0., "float32"))))),
            name='A_change')
