...
output_tensor = (batch*2, in_channel, in_size + pad*2, in_size//2 + pad*2)
...
A_ch = te.compute(
    output_tensor,
        lambda n, c, h, w: tvm.tir.if_then_else(
            n%2 == 0, tvm.tir.if_then_else(tvm.tir.all(w>=pad, h>=pad, h<in_size+pad), A[n//2, c, h-pad, w-pad], tvm.tir.const(0., "float32")), tvm.tir.if_then_else(
            tvm.tir.all(w<in_size//2+pad, h>=pad, h<in_size+pad), A[n//2, c, h-pad, w+in_size//2-pad], tvm.tir.const(0., "float32"))),
            name='A_change')
