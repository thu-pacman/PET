...
class CMutator : public Mutator {
    ...
    // subgraphs with same operators and same input and output shapes
    static std::map<uint64_t, std::vector<std::shared_ptr<SubGraph>>>
        l1_mutation_cache;
    // subgraphs with same operators
    static std::map<uint64_t, std::vector<std::shared_ptr<SubGraph>>>
        l2_mutation_cache;
  ...
  public:
    CMutator(bool prune_reciprocity = true);
    ~CMutator() {
  ...
  private:
    ...
    bool clearTensors(size_t threshold);
    ...
    // manage two-levels cache
    void updateCache(SubGraph *sg, std::vector<SubGraph *> &out_graphs);
    bool checkCache(SubGraph *sg, std::vector<SubGraph *> &out_graphs);
    bool storeCache(uint64_t cacheIdx,
                    std::vector<std::shared_ptr<SubGraph>> out_graphs);
    bool loadCache(uint64_t cacheIdx,
                   std::vector<std::shared_ptr<SubGraph>> out_graphs);
