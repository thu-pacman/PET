...
class Layer(nn.Module):
    ...
    def forward(self, x):
        a = self.q(x).reshape((batch, seqlen, head, hidden)).permute(0, 2, 1, 3)
        assert(list(a.size()) == [batch, head, seqlen, hidden])
        b = self.k(x).reshape((batch, seqlen, head, hidden)).permute(0, 2, 3, 1)
        assert(list(b.size()) == [batch, head, hidden, seqlen])
        c = F.softmax(torch.matmul(a, b), dim=3)
        assert(list(c.size()) == [batch, head, seqlen, seqlen])
        d = self.v(x).reshape((batch, seqlen, head, hidden)).permute(0, 2, 1, 3)
        assert(list(d.size()) == [batch, head, seqlen, hidden])
        e = torch.matmul(c, d).permute(0, 2, 1, 3).reshape((batch, seqlen, embed))
        f = F.relu(self.ff1(e))
        g = F.relu(self.ff2(f))
        return x + g
...
for batch in [1, 4, 16, 64]:
    dummy_input = torch.randn(batch, seqlen, embed)
    torch.onnx.export(model, dummy_input, "models/bert.n{}.onnx".format(batch), verbose=False)